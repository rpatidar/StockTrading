...
class RSIMeanReversion(Strategy):
    ...
    def close_day(self, date, instrument_token, backfill=False):
        ...
        adx_ohlc = OHLCV(open=ohlc['open'], close=ohlc['close'], high=ohlc['high'], low=ohlc['low'],
                         volume=ohlc['volume'])
        if inst_data == None:
            inst_data = {
                "numdays": 0,
        ...
        inst_data["adx"].add_input_value(
            OHLCV(open=ohlc['open'], close=ohlc['close'], high=ohlc['high'], low=ohlc['low'], volume=ohlc['volume']))
        ...
        if previous_cached_info != None:
            ...
            if backfill == False and position != None:
                ...
                self.total_pl += pl
                self.starting_amount += (position['entry_price'] - position['exit_price'] + position['entry_price']) * \
                                        position["quantity"]
            ...
            if backfill == False and setup_details is not None:
                ...
                if position is None:
                    if last_close is not None and setup_details['setup'] and \
                        ...
                        quantity = int(
                            (self.starting_amount / (factor + 1)) / setup_details['sell_trigger_limit_price'])
                        if quantity > 1:
                            position = {
                            ...
                            self.starting_amount = self.starting_amount - setup_details[
                                'sell_trigger_limit_price'] * quantity
                    ...
                    if position.get("exit_price") is None:
                        ...
                        # Ignore this section this is created for pyramidding
                        elif factor > 0 and position["pyramidding_done"] == False and position["entry_price"] * 1.025 < \
                                trading_data['high']:
                            #
                            ...
                            if self.starting_amount > position["entry_price"] * 1.025 * position["quantity"] * factor:
                                self.starting_amount = self.starting_amount - position["entry_price"] * 1.025 * factor * \
                                                       position["quantity"]
                                position["entry_price"] = position["entry_price"] * ((1 + factor * 1.025)) / (
                                            factor + 1)
