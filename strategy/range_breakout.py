...
class RangeBreakout(Strategy):
    ...
    def close_day(self, date, instrument_token, backfill=False):
        ...
        if position is not None and position.get('exit_price') is None:
            #print("Day Closing exit")
            history = self.cache[instrument_token]
            last_date = sorted(history.keys())[-1]
            position['exit_price'] = self.cache[instrument_token][last_date][-1]['close']
    ...
    def run(self, ticks, timestamp, backfill=False):
        ...
        for tick_data in ticks:
            ...
            instCache = self.cache.get(instrument_token)

            if instCache is None:
                instCache = {}

                self.cache[instrument_token] = instCache
            day_history = instCache.get(current_date)
            if day_history is None:
                day_history = []
                instCache[current_date] = day_history
            day_history.append(tick_data['ohlc'])

            symbol = service.get_symbol_and_exchange(instrument_token)

            stock_history = self.cache[instrument_token]
            last_trading_data = None
            current_day_trading_data = instCache[current_date]
            if(len(stock_history.keys()) > 2):
                last_trading_date = sorted(stock_history.keys())[-2]
                last_trading_data = instCache[last_trading_date]


            # print("LastTrading:" + str(last_trading_data))
            # print("CurrentTrading:" + str(current_day_trading_data))
            if first_candle:
                continue
            ...
            if position is None:
                ...
                if last_trading_data != None:
                    ...
                    if tick_data['ohlc']['high'] > lh * 1.01 and vol_today > lv * 2:
                        #print("last-high:" , lh)
                        #print(vol_today ,lv * 2)
                        #print("Buy breakout")
                        #quantity = int(1000 / ((lh - ll) / 2))
