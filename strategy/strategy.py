...
class Strategy:
    ...
    def _get_create_or_get_day_history(self, script, timestamp, readonly=True,  last_aggregate_date=None, aggregate=None,agg_type=1, last_aggregate_time=None ):
        date = timestamp.date()
        ...
        script_history = self.market_history.setdefault(script, {"1minute": OrderedDict, "5minute" : OrderedDict()})[str(agg_type)+"minute"]
        ...
        if aggregate != None:
            ...
            if len(trading_data['trading_data']) > 0 and trading_data["trading_data"][-1]["date"] == last_aggregate_time:
    ...
    def _update_local_cache(self , tick_data, timestamp, agg_type=1):
        ...
        last_stock_trading_data = self.get_simple_day_history(last_aggregate_date,  instrument_token, last_aggregate_time=last_aggregate_time, agg_type=agg_type)
        if last_stock_trading_data != None:
            self._get_create_or_get_day_history(instrument_token, timestamp, readonly=False, last_aggregate_date=last_aggregate_date, aggregate=last_stock_trading_data[0], agg_type=agg_type, last_aggregate_time=last_aggregate_time)
        pass
