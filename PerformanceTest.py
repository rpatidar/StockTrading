...
day_data = []
for d in data:
    if data[0]['date'].replace(hour=0, minute=0, second=0, microsecond=0) == data[0]['date'].replace(hour=0, minute=0, second=0, microsecond=0):
        day_data.append(d)


d = "low,high,open,close" + '\n'.join([str(dd['low'])+","+str(dd['high']) + "," + str(dd['open']) +"," + str(dd['close']) for dd in day_data])
#print(d)
